...
import type { Product, ProductsField, ProductsResponse, ProductsSearchParams } from '@/features/product/types'
...
export async function ProductList({ productsSearchParams, categories }: Props) {
  ...
  let products: ProductsResponse<Pick<Product, ProductsField>> | null = null
  ...
  try {
    ...
    products = productsResponse.payload
  } catch (error: any) {
  ...
  return (
    <div className="pb-7 lg:pb-14">
      ...
      {products ? (
        <PaginationWithLinks
          page={products.paginate.currentPage}
          pageSize={products.paginate.perPage}
          totalPage={products.paginate.totalPage}
        />
      ) : null}
    </div>
  )
