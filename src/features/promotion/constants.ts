import { addDays, format, formatISO, lastDayOfMonth, startOfMonth, subDays } from 'date-fns'

import { PromotionServerItem } from '@/features/promotion/types'

export const PROMOTION_KEY = {
  PROMOTION_DETAIL_FROM_BACKEND: 'promotionDetailFromBackend',
  PROMOTIONS_FROM_SERVER: 'promotionsFromServer',
}

export const MOCK_PROMOTIONS: PromotionServerItem[] = [
  {
    ...
    status: 'active',
    image: '/images/promotion/sale25.png',
  ...
  },
  {
    ...
    status: 'active',
    image: '/images/promotion/sale50.png',
    iat: formatISO(startOfMonth(new Date())),
    exp: formatISO(lastDayOfMonth(new Date())),
  },
  {
    ...
    status: 'expired',
    image: '/images/promotion/free-ship.png',
  ...
  },
  {
    ...
    status: 'upcoming',
    image: '/images/promotion/buy-one-get-one.png',
  ...
  },
  {
    ...
    status: 'inactive',
    image: '/images/promotion/sale10.png',
    iat: formatISO(startOfMonth(new Date())),
    exp: formatISO(lastDayOfMonth(new Date())),
  },
  {
    ...
    status: 'expired',
    image: '/images/promotion/free-ship-3.png',
    iat: formatISO(subDays(new Date(), 10)),
    exp: formatISO(subDays(new Date(), 2)),
  },
  {
  ...
  {
    _id: crypto.randomUUID(),
    code: format(new Date(), 'MMMM').toUpperCase(),
    title: 'Tháng ưu đãi',
    type: 'discount',
    conditions: [
      'Giảm 30% tổng giá trị đơn hàng.',
      'Chỉ áp dụng cho đơn hàng từ 1.000.000đ.',
      'Không áp dụng đồng thời với các mã giảm giá khác.',
    ],
    status: 'upcoming',
    image: '/images/promotion/promo-other.png',
  ...
  },
  {
    ...
    status: 'inactive',
    image: '/images/promotion/sale20.png',
    iat: formatISO(startOfMonth(new Date())),
    exp: formatISO(lastDayOfMonth(new Date())),
  },
]
