...
>(({ className, isCloseIcon = true, isDefaultAnimation = true, children, ...props }, ref) => {
    ...
    <DialogPortal>
      <DialogOverlay>
        <DialogPrimitive.Content
          ref={ref}
          className={cn(
            'relative z-50 my-4 w-full max-w-[calc(100%-2rem)] gap-4 rounded-lg border bg-background p-6 shadow-lg sm:max-w-lg',
            {
              'translate-y-[-55%] scale-[0.97]': context.innerOpen,
        ...
        >
          {children}
          {isCloseIcon ? (
            <DialogClose className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-highlight focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
...
const InnerDialogContent = React.forwardRef<React.ElementRef<typeof DialogPrimitive.Content>, InnerDialogContentProps>(
    ...
    return (
      <DialogPortal>
        <DialogPrimitive.Content
          ...
          className={cn(
            'fixed left-1/2 top-1/2 z-[60] grid w-full max-w-lg -translate-x-1/2 translate-y-[-45%] gap-4 rounded-lg border bg-background p-6 shadow-lg duration-200',
            isClosingByDrag
